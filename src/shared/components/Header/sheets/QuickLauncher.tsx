...
import { useMenu } from "../utils/menu"; // ✅ Utilisation du nouveau hook
...
export default function QuickLauncher({
  ...
  const unreadMessages = (convItems as Conversation[]).reduce((sum, c) => sum + (c.unread || 0), 0);
  ...
  // ✅ APPEL DU HOOK INTELLIGENT
  const rawTools = useMenu("account", { status, roles: user?.roles ?? [] });
  const baseTools = isGuest ? [] : rawTools;
  ...
  async function handleFMMetrixAccess() {
    ...
    try {
      ...
      if (!resp.ok || !(data?.ok && data.allowed)) {
    ...
    >
      ...
      <div className="mt-1 space-y-2">
        ...
        <button
          ...
          className={`hidden max-[405px]:flex w-full rounded-2xl px-3.5 py-3 ring-1 ring-skin-border/15 bg-skin-surface/80 hover:bg-skin-surface/95 transition-colors items-center justify-between gap-3 ${isActiveLink("/discussions") ? "bg-violet-600 text-white" : ""}`}
        >
          <span className="flex items-center gap-3">
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl ring-1 ring-skin-border/15 bg-[rgb(var(--tile))] relative">
              ...
              {unreadMessages > 0 && (
                <span className="absolute -top-1 -right-1 inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1.5 rounded-full bg-red-500 text-white text-[10px] font-semibold shadow-md">
            ...
            {tools.map((i) => {
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`group overflow-hidden rounded-2xl px-4 py-3 text-sm flex items-center gap-2 w-full ${fmGradient} text-white shadow-lg shadow-violet-500/35 transition-transform duration-150 hover:scale-[1.015] ${redirectingFM ? "cursor-wait" : ""}`}
                    >
                      <span
                        aria-hidden
                      ...
                      {!redirectingFM ? (
                        <>
                          ...
                          <span className="relative z-10 ml-auto inline-flex items-center gap-1">
                            ...
                            <span
                              className={`inline-flex w-5 h-5 items-center justify-center rounded-full bg-white/10 transition-transform ${fmMenuOpen ? "rotate-180" : ""}`}
                            >
                    ...
                    {fmMenuOpen && !redirectingFM && (
                      <div
                        className={`absolute left-0 top-[calc(100%+0.5rem)] w-full rounded-2xl overflow-hidden ${fmGradient} shadow-xl shadow-black/25 backdrop-blur-sm z-[80]`}
                      >
                        <button
                          type="button"
                          onClick={() => {
              ...
              return (
                <NavLink
                  ...
                  className={`${base} ${isActive ? "bg-violet-600 text-white" : ""}`}
                >
                  <div className="flex items-center gap-2">
                    {i.icon}

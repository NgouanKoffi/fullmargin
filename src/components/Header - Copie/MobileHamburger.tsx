...
import { LayoutDashboard, Crown } from "lucide-react"; // 👈 on ajoute Crown
...
type Props = {
  ...
  /** ✅ nouveau : total des notifs communautées (demandes reçues + réponses) */
...
export default function MobileHamburger({
  ...
  // ✅ même logique que DesktopActions
  ...
  const openAuth = (mode: "signin" | "signup" = "signin") => {
    // Fermer le drawer AVANT d’ouvrir l’auth
    emitCloseDrawer();
  ...
  const handleOpenLauncher = () => {
    if (!isAuthed) return openAuth("signin");
    emitCloseDrawer();
    window.dispatchEvent(new CustomEvent("fm:open-quick-launcher"));
    onOpenLauncher?.();
  };
  ...
  const btn =
    "inline-flex items-center justify-center rounded-full p-2 ring-1 ring-skin-border/20 " +
    "supports-[backdrop-filter]:bg-skin-header/40 bg-skin-header/60 backdrop-blur-md " +
    "text-skin-base hover:bg-skin-surface transition-colors focus:outline-none " +
    "focus-visible:ring-2 focus-visible:ring-skin-ring";

  // 👇 on décide ici si on affiche le badge sur le burger
  const showCommunityBadge = communityBadgeCount > 0;

  return (
    <div className="min-[1170px]:hidden ml-auto flex items-center gap-2">
      {/* Quick launcher */}
      <button
        type="button"
      ...
      >
        <OrbitRing active size={30} />
        <LayoutDashboard className="w-5 h-5 relative z-[1]" />
      </button>
      ...
      >
        ...
        {/* 🔴 badge notif communautés (sur le bouton du menu) */}
        {showCommunityBadge ? (
          <span

// src/pages/journal/tabs/strategy/StrategyCard.tsx
import { ChevronRight, Pencil, Trash2, Target } from "lucide-react";
import { type Currency, findCurrency } from "../../types";
...
function fmtMoneyLocal(n: number, currency: Currency) {
  const c = findCurrency(currency);
  return (
    ...
    <article className="group rounded-2xl bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-800 shadow-sm overflow-hidden hover:shadow-md transition">
      ...
      <header className="flex flex-wrap gap-3 p-4 border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-[160px] shrink-0">
          ...
          <div className="h-10 w-10 rounded-xl grid place-items-center bg-gradient-to-br from-emerald-500 to-lime-500 text-white shadow-sm">
            <Target className="w-5 h-5" />
          </div>

          <div className="min-w-0">
            {/* pas de truncate sec → on laisse passer sur 2 lignes */}
        ...
        {/* bloc droite : doit descendre en premier quand ça serre */}

...
import { ScrollTabs } from "@features/community/components/ScrollTabs";
import { TABS, type TabKey } from "@features/community/public/layout/tabs.constants";
...
import { getUnseenNotificationsCount } from "@features/community/api/notifications.service";
...
export default function CoursePublic() {
  ...
  const {
    ...
    primaryCtaClick,
    playerHref,
    // ✅ Récupération des infos Modal & Handlers
    paymentModalOpen,
    closePaymentModal,
  ...
  // Conversion de type sûre pour le modal
  const safeLoadingMethod: ModalPaymentMethod =
    paymentLoadingMethod === "card" || paymentLoadingMethod === "feexpay"
      ? paymentLoadingMethod
      : null;
  ...
  return (
    <>
      ...
      {/* MODAL PAIEMENT : Pas de crypto, uniquement Carte et Mobile Money */}
      <PaymentMethodModal
        open={paymentModalOpen}
        onClose={closePaymentModal}
        loadingMethod={safeLoadingMethod}
        disabled={isBusy}
        onCard={handlePayCard}
        onFeexPay={handlePayFeexPay}
        onFeexPaySuccess={handleFeexPaySuccess}
        onSubmitCrypto={() => {}} // Vide car crypto désactivé
        feexpayAvailable={true}
        cryptoAvailable={false} // 🚫 DESACTIVÉ
      />
    </>
  );
}

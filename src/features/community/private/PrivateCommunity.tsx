...
import MobileStickyBar from "@features/community/components/MobileStickyBar";
import { tabLabel, type TabKey } from "@features/community/types";

import { useAuth } from "@core/auth/AuthContext";
import { useCommunityDetails } from "@features/community/hooks/useCommunityDetails";
import { useCommunityMembership } from "@features/community/hooks/useCommunityMembership";

import CommunityDesktopTabs from "@features/community/private/layout/CommunityDesktopTabs";
import CommunityContentSwitch from "@features/community/private/layout/CommunityContentSwitch";
import {
  getRequestCounters,
  leaveCommunity,
} from "@features/community/api/requests.service";
import {
  getReviewCounters,
  markReviewNotificationsAsSeen,
} from "@features/community/api/reviews.service";
import { getUnseenNotificationsCount } from "@features/community/api/notifications.service.ts";
import MobileDrawer from "@features/community/private/layout/MobileDrawer.tsx";
...
export default function CommunityDetails() {
  ...
  // ❌ On ne récupère plus isOwner et isSelfSpace depuis le hook (car trop permissifs)
  const { community, loading, error, requireAuth, joinFromDetails } =
    useCommunityDetails({
      slug,
      status,
      user,
  ...
  // ✅ CORRECTION MAJEURE : On recalcule de manière ultra-stricte la propriété
  ...
  const isSelfSpace = isOwner; // L'espace personnel équivaut à "Je suis le proprio de cette commu"
  ...
  const isDeleted = Boolean(
    community &&
      (community.deletedAt ||
        community.status === "deleted_by_owner" ||
        community.status === "deleted_by_admin")
  );

  const layoutClass = sidebarCollapsed || isDeleted || !!error

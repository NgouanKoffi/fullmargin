...
import type { CommunityCreateFiles, CommunityCreatePayload } from "@features/community/types";
...
function mapCommunity(
  ...
  return {
    ...
    status: String((first as { status?: unknown })?.status || "active"),
    deletedAt: (first as { deletedAt?: unknown })?.deletedAt ? String((first as { deletedAt?: unknown })?.deletedAt) : null,
    isOwner: Boolean((first as { isOwner?: unknown })?.isOwner),
    ownerId: (first as { ownerId?: unknown })?.ownerId ? String((first as { ownerId?: unknown })?.ownerId) : undefined,
  };
}
...
export async function fetchMyCommunity(): Promise<CommunityMinimal | null> {
  ...
  const myOwnCommunity = items.find((c: any) => c.isOwner === true);
  return mapCommunity(myOwnCommunity as Record<string, unknown> | undefined);
}

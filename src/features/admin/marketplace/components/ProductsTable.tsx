...
import { ChevronDown, BadgeCheck, X, Store, Loader2 } from "lucide-react";
...
export default function ProductsTable({
  ...
  onRemove: (id: string, reason: string) => Promise<void> | void;
  ...
  useEffect(() => {
    ...
    const filterParam = (params.get("filter") ||
      params.get("tab")) as FilterType;
    if (
      filterParam &&
      ["published", "pending", "rejected", "suspended", "featured"].includes(
        filterParam,
      )
    ) {
  ...
  const confirmReason = async (reason: string) => {
    ...
    try {
      ...
      if (reasonAction === "reject") {
        if (onReject) await onReject(reasonForId, reason);
      } else if (reasonAction === "suspend") {
        await onSuspend(reasonForId, reason);
      } else if (reasonAction === "delete") {
        await onRemove(reasonForId, reason);
      }
      closeReason();
    } finally {
      setReasonBusy(false);
  ...
  async function handleFeatureToggle(id: string, next: boolean) {
    setFeatureBusy((prev) => new Set(prev).add(id));
    ...
    <section className="rounded-2xl bg-white dark:bg-neutral-900 ring-1 ring-black/10 dark:ring-white/10 p-4 md:p-5 pb-8">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {groups.length === 0 && (
            <div className="py-10 text-center opacity-60">Aucun produit.</div>
          )}
          {groups.map((g) => {

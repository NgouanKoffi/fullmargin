...
export default function FiltersBar() {
  ...
  // Synchronisation lors de la navigation (Précédent/Suivant)
  useEffect(() => {
  ...
  // Génération des badges (Chips) pour affichage
  const chips = useMemo(() => {
    const out: string[] = [];
    const m = toInt(params.get("min"));
    const M = toInt(params.get("max"));
  ...
  return (
    <>
      ...
      <div className="hidden md:block mt-3">
        <div className="p-1.5 flex flex-wrap items-center gap-4 bg-white/60 dark:bg-neutral-900/60 rounded-xl ring-1 ring-black/5 dark:ring-white/5 backdrop-blur-sm shadow-sm">
          {/* Prix */}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              onClick={apply}
              className="h-8 px-4 rounded-lg bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-xs font-bold uppercase tracking-wide hover:shadow-md hover:scale-105 transition-all"
            >
              Filtrer
            </button>
          </div>
        ...
        {/* ✅ Utilisation des "chips" pour corriger TS6133 */}
        {chips.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {chips.map((chip, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-medium rounded-full border border-indigo-100 dark:border-indigo-500/20"
              >
                {chip}
              </div>
            ))}
      ...
      {/* Portal pour FAB + Drawer */}
      {createPortal(
        <>
          <button
            type="button"
            onClick={openAndFocus}
            className="fixed right-4 bottom-4 z-20 md:hidden inline-flex items-center justify-center w-12 h-12 rounded-full bg-neutral-900 text-white ring-1 ring-white/10 shadow-lg"
            aria-label="Filtres"
            title="Filtres"
          >
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-black/10 dark:border-white/10">
              <div className="flex items-center gap-2">
                <div className="inline-flex items-center justify-center rounded-lg bg-indigo-600 text-white w-8 h-8 font-bold">
              ...
              <button
                type="button"
                onClick={close}
                className="inline-flex items-center justify-center rounded-full w-9 h-9 hover:bg-black/5 dark:hover:bg-white/5"
                aria-label="Fermer"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-4 space-y-6">
              <section className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="number"
                    inputMode="numeric"
                    className="h-10 w-full rounded-xl px-3 text-sm bg-neutral-50 dark:bg-neutral-900 ring-1 ring-black/10 dark:ring-white/10"
                    value={min}
                    onChange={(e) => setMin(e.target.value)}
                    placeholder="Min 0 $"
                  />
              ...
              <section className="space-y-3">
                <h4 className="text-sm font-semibold">Trier par</h4>
                <select
                  className="h-10 w-full rounded-xl px-3 text-sm bg-neutral-50 dark:bg-neutral-900 ring-1 ring-black/10 dark:ring-white/10"
                  value={sort}
                  onChange={(e) => setSort(e.target.value)}
                ...
                <div className="text-[11px] opacity-60 flex items-center gap-1.5 px-1">
                  ...
                  Tri combiné avec la note pour les meilleurs résultats.
                </div>
              </section>
            </div>

            {/* Footer Drawer */}
            <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-black/10 dark:border-white/10 flex items-center gap-3 bg-white dark:bg-neutral-950">
              <button
                onClick={clear}
                className="flex-1 h-11 rounded-xl text-sm font-medium ring-1 ring-black/10 dark:ring-white/10 hover:bg-neutral-50 dark:hover:bg-neutral-900"
              >
                Effacer
              </button>
              <button
                ...
                className="flex-[2] h-11 rounded-xl text-sm font-bold bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 shadow-lg"
              >
                Appliquer
              </button>
            </div>

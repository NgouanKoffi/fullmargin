...
export default function OrdersTab() {
  ...
  // ✅ NOUVEAU : État pour gérer le produit à mettre en surbrillance
  ...
  const isFeexPaySuccess =
    new URLSearchParams(window.location.search).get("provider") === "feexpay";
  ...
  return (
    <div className="min-w-0">
      ...
      {refreshingStripe && !isFeexPaySuccess && (
        <div className="mb-3 text-xs rounded-lg px-3 py-2 bg-blue-100 text-blue-800 flex items-center gap-2">
          <div className="w-3 h-3 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
          Synchronisation du paiement Carte Bancaire…
        </div>
      )}

      {(refreshingStripe || loading) && isFeexPaySuccess && (
        <div className="mb-3 text-xs rounded-lg px-3 py-2 bg-emerald-100 text-emerald-900 flex items-center gap-2">
          <div className="w-3 h-3 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
          Finalisation de votre achat Mobile Money…
        </div>
      )}

      {refreshingNowpayments && (
        <div className="mb-3 text-xs rounded-lg px-3 py-2 bg-amber-100 text-amber-900 flex items-center gap-2">
      ...
      {/* On gère le clic sur les onglets pour réinitialiser le highlightId au besoin */}
      <SubtabsBar
        subtab={subtab}
        setSubtab={(tab) => {
          setSubtab(tab);
          if (tab !== "downloads") setHighlightId("");
        }}
      />

      {err && (
        <div className="mb-3 rounded-lg px-3 py-2 text-sm bg-rose-100 text-rose-800">
          {err}
        </div>
      )}
      ...
      {loading ? (
        ...
        <OrdersReceiptsList
          orders={filtered}
          // ✅ Action lors du clic sur le bouton Voir
          onViewProduct={(pid: string) => {
            setHighlightId(pid);
            setSubtab("downloads");
          }}
        />
        ...
        <DownloadsList
          ...
          highlightId={highlightId} // ✅ On passe l'ID à la liste
        />
      ) : (
        <SubscriptionsTab
          orders={filtered}

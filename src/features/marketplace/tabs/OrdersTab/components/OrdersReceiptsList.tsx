...
export default function OrdersReceiptsList({
  ...
  // ✅ On ajoute la prop pour recevoir l'action du parent
    ...
    <ul className="grid gap-3 min-w-0">
      {orders.map((o) => {
        ...
        return (
          <li
            key={o.id}
            className="rounded-2xl ring-1 ring-black/10 dark:ring-white/10 bg-white/70 dark:bg-neutral-900/60 p-4 min-w-0 hover:bg-white dark:hover:bg-neutral-900 transition-colors"
          >
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 min-w-0">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 min-w-0">
                  ...
                  <span
                    className={`text-[11px] px-2 py-0.5 rounded-full ring-1 font-medium tracking-wide ${
                ...
                <div className="text-xs mt-2 min-w-0">
                  Produit(s) :
                  <ul className="mt-1.5 ml-4 list-disc min-w-0 space-y-1.5">
                    {(o.items || []).map((it, idx) => {
                      const name =
                      ...
                      return (
                        <li
                          key={`${it.product}-${idx}`}
                          className="break-words flex flex-wrap items-center gap-1.5"
                        >
                          ...
                          {onViewProduct && pid && (
                            <button
                              ...
                              className="ml-1 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 text-slate-600 hover:bg-violet-100 hover:text-violet-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-violet-900/30 dark:hover:text-violet-300 transition-colors"
                          ...
                          {promoUsed && (
                            <span className="ml-1 inline-flex flex-wrap items-center gap-1 text-[10px] font-semibold rounded px-1.5 py-0.5 ring-1 ring-emerald-300/70 text-emerald-800 dark:text-emerald-300 dark:ring-emerald-800/70 bg-emerald-50 dark:bg-emerald-900/20">
                ...
                {codes.length > 0 && (
                  <div className="text-[11px] mt-2 opacity-80 break-words">
              ...
              <div className="shrink-0 text-left md:text-right mt-1 md:mt-0">
                <div className="text-sm font-bold text-violet-700 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/10 px-3 py-1.5 rounded-lg inline-block md:block border border-violet-100 dark:border-violet-900/30">

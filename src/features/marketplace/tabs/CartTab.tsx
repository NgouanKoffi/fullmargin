...
import { useCartItems, cartActions } from "@features/marketplace/lib/cart";
import { wishlistActions } from "@features/marketplace/lib/wishlist";
  ...
  const qtySig = useMemo(
    () =>
      raw
        ...
        .map((r: { id: string; qty: number }) => `${r.id}:${Math.max(0, Number(r.qty) || 0)}`)
        .sort()
        .join("|"),
    [raw, hiddenIds]
  );

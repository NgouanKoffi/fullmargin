// src/pages/marketplace/CheckoutPage.tsx
...
export default function CheckoutPage() {
  ...
  const {
    ...
    handlePayCrypto,
    handlePayFeexPay,
    handleFeexPaySuccess, // ✅ ON RECUPERE LA FONCTION ICI !
    isRenewCheckout,
    renewProductId,
    cryptoEligible,
  ...
  const modalLoadingMethod: ModalPaymentMethod =
    paymentLoadingMethod === "card" ||
    paymentLoadingMethod === "crypto" ||
    paymentLoadingMethod === "feexpay"
      ? paymentLoadingMethod
      : null;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 lg:px-6 pb-16">
        ...
        <section className="rounded-2xl ring-1 ring-black/10 dark:ring-white/10 bg-white/70 dark:bg-neutral-900/60 p-4 md:p-5">
          ...
          <div className="rounded-xl border border-dashed border-black/15 dark:border-white/15 p-4 text-sm opacity-80">
            {isFreeOrder
              ...
              : stripeReady
                ? cryptoEligible
                  ? "Choisissez entre Carte bancaire, Mobile Money ou Crypto-monnaie."
                  : "Choisissez entre Carte bancaire ou Mobile Money."
      ...
      <PaymentMethodModal
        open={isFreeOrder ? false : paymentModalOpen}
        onClose={closePaymentModal}
        onCard={() => {
          void handlePayCard();
        }}
        onFeexPay={handlePayFeexPay}
        onFeexPaySuccess={handleFeexPaySuccess} // ✅ ON PASSE LA FONCTION AU MODAL
